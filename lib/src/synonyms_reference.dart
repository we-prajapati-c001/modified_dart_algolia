...
class AlgoliaSynonyms {
  ...
  const AlgoliaSynonyms({
    ...
    this.forwardToReplicas = false,
  })  : assert((type == SynonymsType.synonym && synonyms != null) || type != SynonymsType.synonym,
            '`synonyms` (array of strings): Words or phrases to be considered equivalent.'),
        assert(
            (type == SynonymsType.onewaysynonym && synonyms != null && input != null) ||
                type != SynonymsType.onewaysynonym,
            ' - `input` (string): Word or phrase to appear in query strings. \n - `synonyms` (array of strings): Words or phrases to be matched in records.'),
        assert(
            (type == SynonymsType.altcorrection1 && corrections != null && word != null) ||
        ...
        assert(
            (type == SynonymsType.altcorrection2 && corrections != null && word != null) ||
                type != SynonymsType.altcorrection2,
            '- `word` (string): Word or phrase to appear in query strings. \n - `corrections` (array of strings): Words to be matched in records. Phrases (multiple-word synonyms) are not supported.'),
        assert(
            (type == SynonymsType.placeholder && replacements != null && placeholder != null) ||
  ...
  factory AlgoliaSynonyms.fromMap(Map<String, dynamic> map) {
    return AlgoliaSynonyms(
      ...
      type: EnumUtil.fromStringEnum<SynonymsType>(SynonymsType.values, map['type']),
      synonyms: map['synonyms'] != null ? List<String>.from(map['synonyms']) : null,
      corrections: map['corrections'] != null ? List<String>.from(map['corrections']) : null,
      replacements: map['replacements'] != null ? List<String>.from(map['replacements']) : null,
      input: map['input'],
      word: map['word'],
  ...
  factory AlgoliaSynonyms.fromJson(String source) => AlgoliaSynonyms.fromMap(json.decode(source));
