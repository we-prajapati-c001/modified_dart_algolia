...
class AlgoliaIndexReference extends AlgoliaQuery {
  ...
  Future<List<AlgoliaObjectSnapshot>> getObjectsByIds([List<String> objectIds = const []]) async {
    final objects = List<Map>.generate(
        objectIds.length, (int i) => {'indexName': index, 'objectID': objectIds[i]});
    var response = await algolia._apiCall(
      ApiRequestType.post,
      'indexes/*/objects',
...
class AlgoliaMultiIndexesReference {
  ...
  Future<List<AlgoliaQuerySnapshot>> getObjects() async {
    ...
    for (var query in _queries!) {
      var q = query;
      if (q.parameters.containsKey('minimumAroundRadius')) {
        assert(
            (q.parameters.containsKey('aroundLatLng') ||
                q.parameters.containsKey('aroundLatLngViaIP')),
            'This setting only works within the context of a circular geo search, enabled by `aroundLatLng` or `aroundLatLngViaIP`.');
      }
      if (q.parameters['attributesToRetrieve'] == null) {
        q = q._copyWithParameters(<String, dynamic>{
          'attributesToRetrieve': const ['*']
        });
      }
      requests.add({
        'indexName': query._index,
        'params': _encodeMap(query.parameters),
      });
    }
