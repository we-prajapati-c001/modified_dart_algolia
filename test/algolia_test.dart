...
class Application {
  ...
  static Algolia get algolia {
    _dotEnv.load();
    return Algolia.init(
      applicationId: _dotEnv.getOrElse('ALGOLIA_APP_ID', () => ''),
      apiKey: _dotEnv.getOrElse('ALGOLIA_API_KEY', () => ''),
    );
  }
}
...
void main() async {
  ...
  test('Test Environment For Key', () {
    print('ALGOLIA_APP_ID: ${dotEnv.getOrElse('ALGOLIA_APP_ID', () => '')}');
    print('ALGOLIA_API_KEY: ${dotEnv.getOrElse('ALGOLIA_API_KEY', () => '')}');
    expect(dotEnv.isDefined('ALGOLIA_APP_ID'), true);
    expect(dotEnv.isDefined('ALGOLIA_API_KEY'), true);
  });
  ...
  group('index operations', () {
    ...
    test('Perform Get Object to existing Index.', () async {
      try {
        addedObject =
            await algolia.instance.index('contacts').object(taskAdded.data['objectID']).getObject();
      } catch (err) {
    ...
    test('Perform Delete Object to existing Index.', () async {
      taskDeleted =
          await algolia.instance.index('contacts').object(addedObject.objectID).deleteObject();
    ...
    test('Perform Copy index', () async {
      var task = await algolia.instance.index('contacts').copyIndex(destination: 'contacts_alt');
    ...
    test('Perform Multiple Queries', () async {
      ...
      try {
        // Get Result/Objects
        var snap = await algolia.multipleQueries.addQueries([queryA, queryB]).getObjects();
    ...
    test('10. Get Settings of "contacts" index', () async {
      var settings = await algolia.instance.index('contacts').settings.getSettings();
  ...
  group('querying', () {
    ...
    test('Perform Query with page and Hits Per Page', () async {
      var query = algolia.instance.index('contacts').setPage(1).setHitsPerPage(10);
  ...
  group('attributes', () {
    test('Perform SearchableAttributes', () async {
      var settings = algolia.instance.index('contacts').settings.setSearchableAttributes(['name']);
    ...
    test('Perform UnretrievableAttributes', () async {
      var settings =
          algolia.instance.index('contacts').settings.setUnRetrievableAttributes(['isDelete']);
    ...
    test('Perform AttributesToRetrieve', () async {
      var settings = algolia.instance.index('contacts').settings.setAttributesToRetrieve(['email']);
    ...
    test('Perform RestrictSearchableAttributes', () async {
      var settings = algolia.instance.index('contacts').settings.setSearchableAttributes(['name']);
      var query = algolia.instance.index('contacts').setRestrictSearchableAttributes(['name']);
  ...
  group('ranking', () {
    test('Perform Ranking', () async {
      var settings = algolia.instance.index('contacts').settings.setRanking(['words']);
    ...
    test('Perform custom ranking', () async {
      var settings =
          algolia.instance.index('contacts').settings.setCustomRanking(['asc(createdAt)']);
    ...
    test('Perform Numeric Filter', () async {
      var settings =
          algolia.instance.index('contacts').settings.setNumericAttributesForFiltering(['price']);
      AlgoliaQuery query = algolia.instance.index('contacts');
  ...
  group('highlighting / snippeting', () {
    test('Perform attributes To Highlight', () async {
      var settings =
          algolia.instance.index('contacts').settings.setAttributesToHighlight(['email']);
    ...
    test('Perform attributes To Snippet', () async {
      var settings =
          algolia.instance.index('contacts').settings.setAttributesToSnippet(['contact']);
  ...
  group('typos', () {
    ...
    test('Perform allow typos on numericTokens', () async {
      //Set default typo tolerance mode
      var settings =
          algolia.instance.index('contacts').settings.setAllowTyposOnNumericTokens(false);
    ...
    test('Perform disable typo Tolerance on Attributes', () async {
      ...
      var settings = algolia.instance
          .index('contacts')
          .settings
          .setDisableTypoToleranceOnAttributes(['status']).setSearchableAttributes(['status']);
  ...
  group('language', () {
    ...
    test('Perform decompounded attributes', () async {
      var settings = algolia.instance.index('contacts').settings.setDecompoundedAttributes([
    ...
    test('Perform keep diacritics on characters', () async {
      var settings =
          algolia.instance.index('contacts').settings.setkeepDiacriticsOnCharacters('øé');
  ...
  group('personalization', () {
    test('Perform enable personalization', () async {
      ...
      var settings =
          algolia.instance.index('contacts').settings.setEnablePersonalization(enabled: true);
  ...
  group('query strategy', () {
    ...
    test('Perform disable Prefix On Attributes', () async {
      var settings =
          algolia.instance.index('contacts').settings.setDisablePrefixOnAttributes(['sku']);
    ...
    test('Perform disable exact on attributes', () async {
      var settings =
          algolia.instance.index('contacts').settings.setDisableExactOnAttributes(['email']);
  ...
  group('Algolia Synonyms', () {
    ...
    test('Perform adding synonyms to Algolia Object. (single)', () async {
      ...
      try {
        // single
        AlgoliaTask task = await algolia.index('contacts').synonyms.save(synonyms);
        expect(task.runtimeType, AlgoliaTask);
        print(task.data);
        print('\n\n');
      } on AlgoliaError catch (err) {
    ...
    test('Perform adding synonyms to Algolia Object. (batch)', () async {
      ...
      try {
        // single
        AlgoliaTask task = await algolia.index('contacts').synonyms.batch([synonyms]);
        expect(task.runtimeType, AlgoliaTask);
        print(task.data);
        print('\n\n');
      } on AlgoliaError catch (err) {
    ...
    test('Perform delete Object to existing Index.', () async {
      taskAdded = await algolia.instance.index('contacts').object('1').deleteObject();
      await taskAdded.waitTask();
